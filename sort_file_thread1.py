...
thread_pool = futures.ThreadPoolExecutor(50)
...
def sort_directory(path_for_sorting: Path, new_path: Path):
    for el in path_for_sorting.iterdir():
        if el.is_dir():
            thread_pool.submit(sort_directory, el, new_path)
        elif el.is_file():
...
# create path to directory for file this type
            ...
            thread_pool.submit(move_file, el, new_directory.joinpath(el.name))
...
# MainThread after-before=0.10999846458435059 - result of execution